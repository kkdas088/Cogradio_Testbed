...
class cs_mac(object):
    ...
    def phy_rx_callback(self, ok, payload):
        ...
        """
        global x,final_freq,e,R,z,SYN_ACK   
        if self.verbose:
            print "Rx: ok = %r  len(payload) = %4d" % (ok, len(payload))
        if ok:
        
            #if not (len(payload)==30) and (len(payload)==14):
            ...
            if (len(payload)==30):
                ...
                SYN_ACK = payload
                self.tb.send_pkt(SYN_ACK)
                print "sent SYN_ACK %s" %len(SYN_ACK)
        ...
        if (len(payload)==30):
            ...
            if x != 0:
                self.tb.send_pkt(SYN_ACK)
                print "sent tuning payload again %s" %SYN_ACK
                x=0
            #D.set()
        if (len(payload)==97):
            print "ACK Recieved"
    ...
    def main_loop(self):
        ...
        """
        min_delay = 0.001               
        global x,R,z,SYN_ACK
        while 1:
            
            if not x == 123:
                print "OS Transmission"
            ...
            if R == 1 and z == 0:
                self.tb.send_pkt(SYN_ACK)
                print "Sent SYN_ACK aggggggggggggggggggggggggggggggggggggggggggggggggggggain = ", len(SYN_ACK)
                R = 0
                z = 0  
                x = 0
...
def main():
    ...
    #global changingfreq
    global pd,x,SYN_ACK,final_freq,r,R
    final_freq=0
    SYN_ACK=0
    x=0
    pd =''
